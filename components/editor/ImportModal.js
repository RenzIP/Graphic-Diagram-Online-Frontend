		...
		reader.onload = (e) => {
			try {
				...
				if (file.name.endsWith('.json')) {
					...
					// JSON structure might be an API document {content, view} or direct IR {nodes, edges}
					if (json.content) {
				...
				if (parsedState) {
					...
					// Notify others if connected
					wsClient.send({ type: 'room_state', state: parsedState }); // Basic heuristic to sync full state if needed, though usually each node change is sent. We might need a reload for everyone, but for now local set is fine.
	...
	return (
		<Modal open={open} title="Import Diagram" onClose={onClose} size="md">
			<div className="flex flex-col gap-4 p-6">
				...
				<div className="mt-4 flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-700 bg-slate-800/50 p-8 transition-colors hover:border-indigo-500 hover:bg-slate-800">
					<input 
						type="file" 
						ref={fileInputRef}
						accept=".json,.dsl,.txt"
