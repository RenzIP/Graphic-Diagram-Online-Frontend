...
function getEdgeGeometry(sourceNode, targetNode, edge = {}, bundle = { index: 0, total: 1 }, allNodes = []) {
	...
	// Nodes render on top of edges, so any part of the arrowhead that falls
	// inside the target shape would be hidden. Push the target anchor outward
	// by the arrowhead length (marker is 10 units wide, scaled by strokeWidth)
	// plus a small buffer so the entire marker stays outside the shape.
	const strokeWidth = edge.style?.strokeWidth || 2;
	const arrowPad = edge.markerEnd === 'none' ? 0 : 10 * strokeWidth + 2;

	if (edge.markerEnd !== 'none') {
		const targetCenter = getNodeCenter(targetNode);
