...
function getSmoothPath(source, target, sourcePosition = 'bottom', targetPosition = 'top', bundle = { index: 0, total: 1 }) {
	...
	// For the target control point, approach from the same side as the entry
	// handle so the curve flows naturally into the target from the outside.
	let cp2 = getControlPoint(target, targetPosition, controlPointDistance);
 ...
 */
function routeAroundObstacles(p1, p2, obstacles, depth = 0, orthogonal = false) {
	if (depth > 4 || obstacles.length === 0) return [];

	// Fast path: direct line is clear
	if (segmentIsClear(p1, p2, obstacles)) return [];

	// Build candidate points: obstacle corners plus generous margins so the
	// smoothed curve has room to bend without clipping the obstacle.
	const candidates = [];
...
function getEdgeGeometry(sourceNode, targetNode, edge = {}, bundle = { index: 0, total: 1 }, allNodes = []) {
	...
	// Small outward pad so the arrowhead tip sits just outside the node's
	// stroke and isn't clipped by the shape fill.
	const arrowPad = edge.markerEnd === 'none' ? 0 : 3;

	let target = edge.targetHandle
		? getHandlePoint(targetNode, targetHandle)
		: getShapeConnectionPoint(targetNode, targetAim, arrowPad);
	...
	if (!currentWaypoints?.length && allNodes.length > 0) {
		...
		const autoWaypoints = routeAroundObstacles(routeStart, routeGoal, obstacles, 0, type === 'step');
	...
	if (currentWaypoints?.length) {
		let pts;
		if (type === 'straight') {
			// For straight edges with bundle offset, the waypoints were generated
			// in the shifted source/target space. Build the path from those same
			// shifted endpoints so the line stays parallel to the direct edge.
			if (bundle.total > 1) {
				const spacing = 14;
				const offset = (bundle.index - (bundle.total - 1) / 2) * spacing;
				const dx = targetPadded.x - source.x;
				...
				const px = -dy / len;
				const py = dx / len;
				const s = { x: source.x + px * offset, y: source.y + py * offset };
				const t = { x: targetPadded.x + px * offset, y: targetPadded.y + py * offset };
				pts = [s, ...currentWaypoints, t];
			} else {
