...
/**
 * Recursively find waypoints that route the edge from p1 to p2 around
 * axis-aligned bounding box obstacles. Returns an array of intermediate points.
 */
function routeAroundObstacles(p1, p2, obstacles, depth = 0) {
	...
	for (const obs of obstacles) {
		if (segmentIntersectsBBox(p1, p2, obs)) {
	...
	const corners = [
		{ x: hitObstacle.left, y: hitObstacle.top },
		{ x: hitObstacle.right, y: hitObstacle.top },
		{ x: hitObstacle.left, y: hitObstacle.bottom },
		{ x: hitObstacle.right, y: hitObstacle.bottom }
	];

	let bestCorner = null;
	let minPathDist = Infinity;
	for (const c of corners) {
		const dist = Math.hypot(p1.x - c.x, p1.y - c.y) + Math.hypot(c.x - p2.x, c.y - p2.y);
		if (dist < minPathDist) {
			minPathDist = dist;
			bestCorner = c;
		}
	...
	const leftPath = routeAroundObstacles(p1, bestCorner, obstacles, depth + 1);
	const rightPath = routeAroundObstacles(bestCorner, p2, obstacles, depth + 1);

	return [...leftPath, bestCorner, ...rightPath];
}
...
function getEdgeGeometry(sourceNode, targetNode, edge = {}, bundle = { index: 0, total: 1 }, allNodes = []) {
	...
	if (currentWaypoints?.length) {
		...
		path =
			type === 'straight'
				? [`M ${source.x} ${source.y}`, ...currentWaypoints.map((p) => `L ${p.x} ${p.y}`), `L ${targetPadded.x} ${targetPadded.y}`].join(' ')
