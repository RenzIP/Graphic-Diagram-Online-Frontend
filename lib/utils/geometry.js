...
function getCachedRoute(routeStart, routeGoal, obstacles, orthogonal, maxRadius) {
	...
	const result = routeAroundObstacles(routeStart, routeGoal, obstacles, 0, orthogonal, maxRadius);
...
function getEdgeGeometry(sourceNode, targetNode, edge = {}, bundle = { index: 0, total: 1 }, allNodes = []) {
	...
	if (!currentWaypoints?.length && allNodes.length > 0) {
		...
		const autoWaypoints = getCachedRoute(routeStart, routeGoal, obstacles, type === 'step', 32);
