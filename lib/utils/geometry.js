 ...
 */
function getSmoothPolyline(points, maxRadius = 16) {
	if (points.length < 2) return '';
	...
	for (let i = 1; i < points.length - 1; i++) {
		...
		d += ` Q ${curr.x} ${curr.y} ${after.x.toFixed(2)} ${after.y.toFixed(2)}`;
	}
...
function getEdgeGeometry(sourceNode, targetNode, edge = {}, bundle = { index: 0, total: 1 }, allNodes = []) {
	...
	if (!currentWaypoints?.length && allNodes.length > 0) {
		...
		// Use exit/entry points as routing start/goal so the path doesn't backtrack
		// near the source/target nodes.
		const offset = 24;
		...
		const autoWaypoints = routeAroundObstacles(startExit, endEntry, obstacles, 0, type === 'step');
