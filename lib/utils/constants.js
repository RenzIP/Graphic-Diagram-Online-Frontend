...
// Browsers block insecure ws:// connections opened from an https:// page
// (Mixed Content). Whenever the current page is served over HTTPS, force the
// WebSocket URL to wss:// so a misconfigured env var can't break collaboration.
const enforceSecureWs = (url) => {
...
const WS_BASE_URL = (() => {
  if (process.env.NEXT_PUBLIC_WS_URL) {
    return enforceSecureWs(process.env.NEXT_PUBLIC_WS_URL);
  }
  ...
  return enforceSecureWs(`${wsProtocol}://${cleaned}/ws`);
})();
export {
  ALL_SHAPES,
  API_BASE_URL,
