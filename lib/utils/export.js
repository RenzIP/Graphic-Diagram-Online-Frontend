async function exportPNG(svgElement, state, filename = "diagram.png") {
  ...
  await exportRaster(preparedSvg, width, height, filename, "image/png", "#0f172a");
}
...
async function exportWebP(svgElement, state, filename = "diagram.webp") {
  const { svg: preparedSvg, width, height } = prepareSvgForExport(svgElement, state);
  await exportRaster(preparedSvg, width, height, filename, "image/webp", "#0f172a");
}
...
function prepareSvgForExport(originalSvg, state) {
  ...
  // Embed all document CSS into the SVG so Tailwind classes work in Canvas
  let cssText = '';
  ...
  // Remove UI selection elements but KEEP the grid
  const uiSelectors = [
