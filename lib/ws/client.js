...
class WebSocketClient {
  constructor() {
    ...
    // True once we've connected at least once for this document, so a later
    // successful open is a *reconnect* and must resync the document state.
  ...
  connect(documentId) {
    ...
    collaborationStore.setStatus("connecting");
    this.doConnect();
  }
  doConnect() {
    ...
    try {
      ...
      if (!token) {
        ...
        collaborationStore.setStatus("error");
        return;
      }
      ...
      this.ws.onopen = () => {
        ...
        console.info(`[WS] Connected to document ${this.documentId}${isReconnect ? " (reconnect)" : ""}`);
        collaborationStore.joinRoom(this.documentId);
        this.send({ type: "join_room", room_id: this.documentId });
        // On reconnect we may have missed edits while offline. The server does
        // not replay document state on join, so pull the latest from the API.
        if (isReconnect) this.resyncDocument();
      };
      ...
      this.ws.onclose = (event) => {
        ...
        if (event.code === 1000) {
          collaborationStore.setStatus("idle");
        } else {
          // Abnormal close. code 1006 with no prior open usually means the
          // server rejected the connection (auth/authorization failure).
          if (!this.hasConnected) {
            console.error(`[WS] Connection rejected for document ${this.documentId} (code ${event.code}). Likely not a workspace member or invalid token.`);
          } else {
            console.warn(`[WS] Connection lost (code ${event.code}), reconnecting...`);
          }
          collaborationStore.setStatus("reconnecting");
  ...
  /** Reload document content from the API after a reconnect. */
  resyncDocument() {
    const docId = this.documentId;
    if (!docId) return;
    documentStore.load(docId).then((doc) => {
      if (doc) {
        console.info(`[WS] Resynced document ${docId} after reconnect`);
        if (typeof window !== "undefined") {
          window.__gradiol_toast?.("Reconnected — document synced", "success");
        }
      }
    }).catch((err) => console.error("[WS] Resync failed:", err));
  }
