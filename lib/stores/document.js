...
// Lazily resolve the WS client to avoid a static import cycle
// (ws/client.js imports this store). Cached after first load.
let _wsClient = null;
import('../ws/client.js')
	.then((m) => {
		_wsClient = m.wsClient;
	})
	.catch(() => {});

// Broadcast a local mutation to collaborators. No-op when disconnected.
// Only the *local* mutators call this; the *Remote twins stay silent so
// applying an inbound event never echoes back to the sender.
function broadcast(msg) {
	if (_wsClient) _wsClient.send(msg);
}
...
export const documentStore = {
	...
	// Replace the whole document (import, version restore) and broadcast it
	// so collaborators converge. Pushes history so the local user can undo.
	replaceDocument(next) {
		state.update((s) => {
			saveHistory(s);
			return { nodes: next.nodes ?? [], edges: next.edges ?? [] };
		});
		broadcast({ type: 'replace_document', state: { nodes: next.nodes ?? [], edges: next.edges ?? [] } });
	},
	// Apply an inbound whole-document replace from a collaborator (no echo).
	replaceRemote(next) {
		if (!next) return;
		state.set({ nodes: next.nodes ?? [], edges: next.edges ?? [] });
	},
	// Replace the whole document and broadcast it WITHOUT touching history.
	// For undo/redo, which already manage the history stacks themselves —
	// pushing history here would corrupt the redo stack.
	syncDocument(next) {
		if (!next) return;
		state.set({ nodes: next.nodes ?? [], edges: next.edges ?? [] });
		broadcast({ type: 'replace_document', state: { nodes: next.nodes ?? [], edges: next.edges ?? [] } });
	...
	updateNode(id, data) {
		...
		broadcast({ type: 'update_node', node_id: id, changes: data });
	},
	...
	removeNode(id) {
		...
		broadcast({ type: 'delete_node', node_id: id });
	},
	removeNodeRemote(id) {
	...
	updateEdge(id, data) {
		...
		broadcast({ type: 'update_edge', edge_id: id, changes: data });
	},
	...
	removeEdge(id) {
		...
		broadcast({ type: 'delete_edge', edge_id: id });
	},
	removeEdgeRemote(id) {
